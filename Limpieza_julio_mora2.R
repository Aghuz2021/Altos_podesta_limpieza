datos <- read.csv("c:\\Users\\Agustin\\Documents\\Programacion\\R\\Podesta\\codigo v2\\Download.csv")
...
datos <- datos %>%
  mutate(fvto = if_else(is.na(fvto), as.Date(fpago), fvto),
         fvto = as.Date(paste(year(fvto), month(fvto), 15, sep = "-")))
...
datos <- datos %>%
  mutate(Pago_con_interes = case_when(
    estado == "Pago con intereses" ~ round(importe_are * 1.0921 * 1.0921 * 1.0921 *1.0921),
    estado == "Pagó a tiempo" ~ round(importe_are),
    estado == "Mora" & (Sys.Date() > fvto) ~ round(importe * 1.0921),
    TRUE ~ NA_real_
  ))
...
write.csv(datos, "datos_exportados3.csv", row.names = FALSE)
write_csv(paste("Exportados","datos_exportados3.csv", row.names = FALSE))